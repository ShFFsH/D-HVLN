...
def train_fn(cfgs: TotalConfigs, model_name: str, model: nn.Module, matcher: HungarianMatcher, train_loader,test_loader, valid_loader, device):
    ...
    for epoch in range(cfgs.train.max_epochs):
        ...
        '''
        #feature2ds, feature3ds, objects, object_masks, vp_semantics, caption_semantics, numberic_caps, masks, captions, nouns, vids, vocab_ids, vocab_probs, fillmasks  14个
        for i, (feature2ds, feature3ds, objects, object_masks, vp_semantics, caption_semantics, numberic_caps, masks, captions, nouns, vids, vocab_ids, vocab_probs, fillmasks) in enumerate(train_loader):
            ...
            '''
                pred: 模型的预测输出。
                target: 目标序列，是模型应该学习的序列。  numberic_caps  独热编码 句子的独热编码
                mask: 掩码，用于指示哪些部分需要计算损失。 masks
                eos_idx: 表示序列结束的索引。  cfgs.dict.eos_idx
            '''
            ...
            # video module loss caption_semantics sbert抽取得到的
            ...
            if cnt % cfgs.train.save_checkpoints_every == 0:
                ...
                if epoch >= 250:
                    eval_fn(model=model, loader=test_loader, device=device,
                                    idx2word=idx2word, save_on_disk=False, cfgs=cfgs,
                                    vid2groundtruth=vid2groundtruth)
